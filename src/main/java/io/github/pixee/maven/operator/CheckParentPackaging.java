...
public class CheckParentPackaging extends AbstractCommand {
    ...
    private boolean packagingTypePredicate(POMDocument d, String packagingType) {
        List<?> elementTextList = Util.selectXPathNodes(d.getPomDocument().getRootElement(), "/m:project/m:packaging/text()");
    ...
    private boolean hasValidParentAndPackaging(POMDocument pomFile) {
        List<?> parentNodes = Util.selectXPathNodes(pomFile.getPomDocument().getRootElement(), "/m:project/m:parent");
        ...
        List<?> packagingNodes = Util.selectXPathNodes(pomFile.getPomDocument().getRootElement(), "/m:project/m:packaging/text()");

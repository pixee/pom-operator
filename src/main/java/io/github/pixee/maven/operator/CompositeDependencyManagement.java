...
public class CompositeDependencyManagement extends AbstractCommand {
    ...
    public boolean execute(ProjectModel pm) {
        ...
        if (parentPomFile.getResultPom().getRootElement().elements("dependencyManagement").isEmpty()) {
            dependencyManagementElement = Util.addIndentedElement(parentPomFile.getResultPom().getRootElement(),
                    parentPomFile,
    ...
    private Element modifyDependency(POMDocument pomFileToModify, String lookupExpressionForDependency, ProjectModel c, Element parentElement, boolean dependencyManagementNode) {
        List<Node> dependencyNodes = Util.selectXPathNodes(pomFileToModify.getResultPom(), lookupExpressionForDependency);
